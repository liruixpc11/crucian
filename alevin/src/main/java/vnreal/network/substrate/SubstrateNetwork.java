...
public final class SubstrateNetwork extends
    ...
    public SubstrateNetwork getCopy(boolean autoUnregister) {
        return getCopy(autoUnregister, false);
    }

    public SubstrateNetwork getCopy(boolean autoUnregister, boolean deepCopy) {
        SubstrateNetwork copySNetwork = new SubstrateNetwork(autoUnregister);
        ...
        for (Iterator<SubstrateNode> tempSubsNode = getVertices()
                .iterator(); tempSubsNode.hasNext(); ) {
            tmpSNode = tempSubsNode.next();
            if (deepCopy) {
                SubstrateNode clone = tmpSNode.clone();
                copySNetwork.addVertex(clone);
                map.put(tmpSNode.getNetworkAddress(), clone);

            } else {
                copySNetwork.addVertex(tmpSNode);
            }
        }
        for (Iterator<SubstrateLink> tempItSubLink = originalLinks.iterator(); tempItSubLink
            ...
            if (deepCopy) {
                copySNetwork.addEdge(tmpSLink,
                        map.get(tmpSNode.getNetworkAddress()),
                        map.get(tmpDNode.getNetworkAddress()));
            } else {
                copySNetwork.addEdge(tmpSLink, tmpSNode, tmpDNode);
            }
        }
