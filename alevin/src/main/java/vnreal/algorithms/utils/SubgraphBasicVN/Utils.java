...
public class Utils {
    ...
    public static boolean occupyResources(
            VirtualLink n1,
            SubstrateNode n2) {
...
//	}
    ...
    public static Collection<ResourceDemandEntry<? extends AbstractResource, ? extends AbstractDemand>> occupyPathResources(
        ...
        for (SubstrateLink e : path) {
            ...
            if (!(sNetwork.getSource(e).equals(sourceSnode)))
                if (!occupyResources(vl, sNetwork.getSource(e)))
    ...
    public static EnergyResource getEnergyResource(SubstrateNode sn) {
        ...
        throw new AssertionError("no energy resource found!");
    }
    ...
    public static double getCpuResource(
    ...
    public static double getCpuDemand(NetworkEntity<? extends AbstractDemand> n) {
        for (AbstractDemand dem : n.get()) {
            if (dem instanceof CpuDemand) {
                return ((CpuDemand) dem).getDemandedCycles();
            }
        }
        return 0.0;
    }
    ...
    public static void generateEnergyDemands(NetworkStack stack) {
        for (int i = 1; stack.getLayer(i) != null; i++) {
            VirtualNetwork vNetwork = (VirtualNetwork) stack.getLayer(i);

            for (VirtualNode vn : vNetwork.getVertices()) {
                vn.add(new EnergyDemand(vn));
            }
        }
    }

    public static void generateHetEnergyConstraints(NetworkStack stack, int min, int max) {
        ...
        for (int i = 1; ToolKit.getScenario().getNetworkStack().getLayer(i) != null; i++) {
            VirtualNetwork vNetwork = (VirtualNetwork) ToolKit.getScenario()
