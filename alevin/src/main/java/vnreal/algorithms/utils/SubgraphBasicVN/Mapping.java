...
public class Mapping {
    ...
    public Mapping(Mapping copy) {
        ...
        for (Entry<SubstrateNode, Collection<VirtualNode>> e : copy.nodeMappings.entrySet()) {
            nodeMappings.put(e.getKey(), new LinkedList<VirtualNode>(e.getValue()));
        }

        nodes = new HashMap<VirtualNode, SubstrateNode>(copy.nodes);

        linkMappings = new HashMap<List<SubstrateLink>, Collection<VirtualLink>>();
        for (Entry<List<SubstrateLink>, Collection<VirtualLink>> e : copy.linkMappings.entrySet()) {
        ...
        mappings = new LinkedList<ResourceDemandEntry<? extends AbstractResource, ? extends AbstractDemand>>(copy.mappings);
    }
    ...
    public Collection<ResourceDemandEntry<? extends AbstractResource, ? extends AbstractDemand>> occupyAllResources(SubstrateNetwork sNetwork) {
        ...
        for (Entry<VirtualLink, List<SubstrateLink>> e : links.entrySet()) {
            mappings.addAll(Utils.occupyPathResources(e.getKey(), e.getValue(), sNetwork, useEnergyDemand));
        }
        return new LinkedList<ResourceDemandEntry<? extends AbstractResource, ? extends AbstractDemand>>(mappings);
    }
    ...
    public void setMappings(Collection<ResourceDemandEntry<? extends AbstractResource, ? extends AbstractDemand>> mappings) {
