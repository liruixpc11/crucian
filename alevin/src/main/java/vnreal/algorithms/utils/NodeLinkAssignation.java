...
public class NodeLinkAssignation {
    ...
    /**
     * @param demands set of demands
     * @param e       Network Entity (Node or link)
     ...
     * @param spath    list of substrate link forming the path
     * @param sNet     substrate network
     * @param srcSNode source substrate node
     * @return boolean value indicating whether mapping has been Successful or
     * not.
     */
     ...
     *
     * @param vl         link to be mapped
     * @param bwDem      percentage of the bandwidth demand of vl to be mapped
     * @param subsLink   substrate link to which the mapping should be done
     * @param hiddenHops boolean value indicating if the cpu demands of hidden hops is
     *                   considered for this virtual link
     * @param hiddenHop  node that is a hidden hop and is the source of subsLink
     * @param hhDemand   is the CPU demand of the hidden hop.
     * @return boolean value indicating whether mapping has been Successful or
     * not.
     */
    public final static boolean vlmSingleLink(VirtualLink vl,
        ...
        for (AbstractResource res : subsLink) {
            if (res instanceof BandwidthResource) {
                ...
                if ((int) ((double) (MiscelFunctions.roundTwoDecimals(tempBwRes
        ...
        for (SubstrateLink tSLink : path) {
            if (!fulfills(vl.get(), tSLink)) {
                ...
                if (!sNet.getSource(tSLink).equals(srcSNode)) {
                    if (!fulfills(vl.getHiddenHopDemands(), sNet
                            .getSource(tSLink)))
                        return false;
                }
            }
