...
public class RoundingPathStrippingLinkMapping extends AbstractLinkMapping {
     ...
     * @param cpuWeight weight assigned to cpu demands
     * @param bwWeight  weight assigned to bandwidth demands
     * @param k         number of considered shortest paths
     * @param type      rounding type (Deterministic or Randomized)
     */
    ...
    protected boolean linkMapping(VirtualNetwork vNet,
        ...
        lpLinkMappingData.createDataSolverFile(sNet, null, vNet, nodeMapping,
                wBw, wCpu, false); // Process all the current
        ...
        if (problemSolver.problemFeasible()) {
            ...
            for (Iterator<VirtualLink> links = vNet.getEdges().iterator(); links
                ...
                if (!srcSnode.equals(dstSnode)) {
                    ...
                    do {
                        // Find a path
                        BreadthFirstSearch bfs = new BreadthFirstSearch(newSNet
                                .keySet(), srcSnode.getId(), dstSnode.getId(),
                                sNet);
                        tSPath = bfs.search();
    ...
    /**
     * @param srcSNode       Source substrate node of the substrate path
     * @param candidatePaths set of candidate paths
     * @param demBW          Bandwidth demanded by the virtual link being processed
