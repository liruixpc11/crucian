 ...
 */
public class SubgraphIsomorphismAlgorithm implements SingleNetworkMappingAlgorithm {
    ...
    Collection<ResourceDemandEntry<? extends AbstractResource, ? extends AbstractDemand>> mapOutEdges(
        ...
        for (VirtualLink vl : g_V.getOutEdges(candidate.t)) {
            ...
            if (sOpposite != null && sOpposite.getId() != candidate.u.getId()) {
                ...
                result.addAll(Utils.occupyPathResources(vl, path, orig_g_P, useEnergyResource));
                m.add(vl, path);
            }
        }
        return result;
...
//							return 1;
    ...
    int compare(MappingCandidate<VirtualNode, SubstrateNode> o1,
        ...
        double tmp = Utils.getCpuResource(o1.u) - Utils.getCpuResource(o2.u);
    ...
    public Mapping mapNetwork(SubstrateNetwork sNetwork,
        ...
        numberOfTries = 0;
        return vnmFlib(new Mapping(useEnergyResource), vNetwork, sNetwork, sNetwork, omega,
                epsilon);
    }
