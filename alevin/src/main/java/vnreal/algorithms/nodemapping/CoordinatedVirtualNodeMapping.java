...
public class CoordinatedVirtualNodeMapping extends AbstractNodeMapping {
    ...
    protected boolean nodeMapping(VirtualNetwork vNet) {
        ...
        Random intGenerator = new Random();// FIXME Temporal solution for
        // executing
        // at the same time different algorithms using
        // this link mapping that must accede to different data files

        // FIXME create a model considering HiddenHops
        ...
        if (augmentedCreation(vNet, unMappedVirtualNodes,
            ...
            lpNodeMappingData.createDataSolverFile(augmentedSubstrate, sNet,
                    vNet, nodeMapping, wCpu, wBw, true);
     ...
     *
     * @param vNet
     * @param unmappedvNodes (updated in the abstract node mapping)
     * @param unmappedsNodes (updated in the abstract node mapping)
     * @param distance
     * @return
     */
    private boolean augmentedCreation(VirtualNetwork vNet,
        ...
        augmentedSubstrate = sNet.getCopy(true, false);
        CpuResource cpuRes;
        ...
        for (Iterator<VirtualNode> vn = unmappedvNodes.iterator(); vn.hasNext(); ) {
            ...
            if (!candidates.isEmpty()) {
                ...
                //FIXME: This value should be modified to be always a very high value
                //in comparison with the values of the substrate network cpu resource.
                ...
                for (Iterator<SubstrateNode> itSubNode = candidates.iterator(); itSubNode
                    ...
                    augmentedSubstrate.addEdge(augSubsLink1,
                            newSubNode, tmpCandidate);
                    ...
                    augmentedSubstrate.addEdge(augSubsLink2,
                            tmpCandidate, newSubNode);
                }
            } else {
                return false;
            }
    ...
    /**
     * Method to find the substrate nodes that can be considered to
     * be mapped to one virtual node
     *
     * @param vNode
     * @param dem
    ...
    /**
     * Method to know if a substrate node is located in a distance less or equal than
     * the predefined distance parameter
     *
     * @param vNode
     * @param sNode
     * @param distance
    ...
    /**
     * This method receives the ouput of the LP solver and performs
     * the node mapping.
     */
    private boolean virtualNodeMapping(List<VirtualNode> unmappedvNodes,
        ...
        for (Iterator<VirtualNode> vn = unmappedvNodes.iterator(); vn.hasNext(); ) {
            ...
            for (Iterator<List<String>> cad = result.keySet().iterator(); cad
                ...
                if ((mappedNode.getId() == Integer.parseInt(tmpValues.get(0)))
                    ...
                    tmpSNode = getSubstrateNode(Integer.parseInt(tmpValues
                            .get(1)));
                    if (candiNodesWeight.get(tmpSNode) != null)
                        candiNodesWeight.put(tmpSNode, vtmp
                                * candiNodesWeight.get(tmpSNode));
                }
            }
            /*
            ...
            if (type == 0) {
				 ...
				 */
                mappedNode = greatest(candiNodesWeight, candiNodes
                        .get(tmpvNode), tmpvNode);
            }
            if (type == 1) {
				/*
				 ...
				 * chosen.
				 */
                mappedNode = randomizedElection(candiNodesWeight, candiNodes
                        .get(tmpvNode), tmpvNode);
            }
            if (mappedNode != null) {
    ...
    /**
     * Return the position of the greatest value of a set, if the resultant value
     * is already mapped, the following greatest value is returned. If there is
     * not feasible node because they have been chosen, a greedy available
     * resources strategy is followed.
     *
     * @param augmented
     * @param flow
     * @param size
    ...
    private Double maxLinkResource() {
        double max = 0;
        for (Iterator<SubstrateLink> links = sNet.getEdges()
            ...
            for (AbstractResource res : temp) {
                if (res instanceof BandwidthResource) {
                    if ((((BandwidthResource) res).getAvailableBandwidth()) >= max)
                        max = (((BandwidthResource) res).getAvailableBandwidth());
                }
            }
