...
public abstract class AbstractNodeMapping {
     ...
     *
     * @param sNet           is the substrate network over which, the mapping
     *                       will be performed.
     * @param unmappedvNodes is the variable that will have the virtual nodes that
     *                       are not mapped after the prenode mapping stage.
     * @param unmappedsNodes is the variable that will have the substrate nodes that
     *                       are not mapped after the prenode mapping stage.
     */
    protected AbstractNodeMapping(SubstrateNetwork sNet, boolean subsNodeOverload) {
    ...
    /**
     * This method realizes the prenode mapping stage. This stage consists in the
     * mapping of virtual nodes requested by their idDemand. Nodes with idDemand
     * should be mapped to nodes with the same idResource.
     *
     * @param vNet
     * @return
     */
        ...
        for (Iterator<VirtualNode> vn = vNet.getVertices().iterator(); vn
            ...
            for (AbstractDemand dem : tmp) {
                if (dem instanceof IdDemand) {
                    numNodesWithIdDemand++;
                    SubstrateNode sNode = findDemandedNode(dem);
                    if (sNode == null) {
                        //FIXME: How to manage this kind of errors (Create a log file?)
                        return false;
    ...
    /**
     * @return a boolean value indicating whether the prenode Mapping phase
     * has mapped all the virtual network nodes
     */
    ...
    /**
     * This method should be implemented by the node mapping algorithms
     * it will have all the logic of the node mapping algorithm.
     *
     * @param vNet is the Virtual network request that should be mapped
     * @return true if the node mapping was successfully performed and
     * false otherwise.
     */
     ...
     * @param IDdem
     * @param tmp
     * @return the substrate node with the IDresource demanded by the
     * virtual node tmp
     */
     ...
     * @param sNode
     * @param vNode
     * @return a boolean value indicating if the demands of virtual node
     * vNode can be accomplished by the substrate node sNode
     */
