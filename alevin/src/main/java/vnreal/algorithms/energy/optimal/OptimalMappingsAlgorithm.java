...
import vnreal.resources.EnergyResource;
...
public class OptimalMappingsAlgorithm extends AbstractAlgorithm {
    ...
    /**
     * @param allowMultipleMappings allow mapping to the same sn multiple times?
     */
    ...
    protected void evaluate() {
        ...
        results.add(new OptimalMappingAlgorithmMappingResult(new Mapping(false),
                null));
        ...
        while (hasNext()) {
            ...
            for (OptimalMappingAlgorithmMappingResult m : results) {
                ...
                vnmFlib(m.mapping, sNetwork.getVertices(), vNetwork
                        .getVertices(), vNetwork, sNetwork, mappingChecker,
                        netResults);
    ...
    void vnmFlib(Mapping m, Collection<SubstrateNode> unmappedSNodes,
        ...
        Collection<ResourceDemandEntry<EnergyResource, EnergyDemand>> demandedEnergyResources = new LinkedList<ResourceDemandEntry<EnergyResource, EnergyDemand>>();
    ...
    void vnmFlib(
            ...
            Collection<OptimalMappingAlgorithmMappingResult> results,
            Collection<ResourceDemandEntry<EnergyResource, EnergyDemand>> demandedNodeEnergyResources) {
        ...
        for (MappingCandidate<VirtualNode, SubstrateNode> candidate : c) {
            ...
            Collection<ResourceDemandEntry<EnergyResource, EnergyDemand>> newDemandedNodeEnergyResources = new LinkedList<ResourceDemandEntry<EnergyResource, EnergyDemand>>(
            ...
            if (edges != null) {
                ...
                for (ResourceDemandEntry<? extends AbstractResource, ? extends AbstractDemand> demEntry : resourceMapping) {
                    if (demEntry.res instanceof EnergyResource) {
                        found = true;
                        newDemandedNodeEnergyResources
                                .add(new ResourceDemandEntry<EnergyResource, EnergyDemand>(
                                        (EnergyResource) demEntry.res,
                                        (EnergyDemand) demEntry.dem));
                    }
                }
                assert (found);
    ...
    Collection<ResourceDemandEntry<? extends AbstractResource, ? extends AbstractDemand>> mapEdges(
            ...
            Mapping m,
            Collection<ResourceDemandEntry<EnergyResource, EnergyDemand>> demandedNodeEnergyResource,
            int epsilon) {

        Collection<ResourceDemandEntry<? extends AbstractResource, ? extends AbstractDemand>> out = mapOutEdges(
                sNetwork, vNetwork, candidate, m,
                epsilon);
        if (out == null) {
            return null;
        }
    ...
    Collection<ResourceDemandEntry<? extends AbstractResource, ? extends AbstractDemand>> mapOutEdges(
        ...
        for (VirtualLink vl : vNetwork.getOutEdges(candidate.t)) {
            ...
            if (sOpposite != null && sOpposite.getId() != candidate.u.getId()) {
                ...
                result.addAll(Utils
                        .occupyPathResources(vl, path, sNetwork, useEnergyDemand));
                m.add(vl, path);
            }
        }
        return result;
