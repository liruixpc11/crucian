...
public class BasicVNAssignmentAlgorithm extends AbstractAlgorithm {
    ...
    /**
     * @param delta_N : default = 1
     * @param delta_L : default = 1
     */
    public BasicVNAssignmentAlgorithm(NetworkStack stack,
    ...
    DistanceEntry getDistance(
            SubstrateNetwork sNetwork, SubstrateNode from, SubstrateNode to,
            Transformer<SubstrateLink, Double> transformer) {

        if (from.getId() == to.getId()) {
            return new DistanceEntry(from, to, new LinkedList<SubstrateLink>(), 0.0);
        }

        // we need a new instance here because we use other link weights
        // every time
        MyDijkstraShortestPath<SubstrateNode, SubstrateLink> dijkstra =
                new MyDijkstraShortestPath<SubstrateNode, SubstrateLink>(sNetwork, transformer, true);
    ...
    public boolean mapNetwork(SubstrateNetwork sNetwork,
        ...
        biggestNodeStress = findBiggestNodeStress(sNetwork, sNetwork.getVertices());
    ...
    double findBiggestLinkStress(SubstrateNetwork sNetwork,
        ...
        for (SubstrateLink sl : links) {
            ...
            if (maxLink == null | (tmp = getStressLevel(sNetwork, endpoints.getFirst(), sl)) > maxNumber) {
                maxNumber = tmp;
                maxLink = sl;
            }
            if (maxLink == null | (tmp = getStressLevel(sNetwork, endpoints.getSecond(), sl)) > maxNumber) {
                maxNumber = tmp;
                maxLink = sl;
            }
        }
    ...
    double findBiggestNodeStress(SubstrateNetwork sNetwork,
        ...
        for (SubstrateNode sn : nodes) {
            if (maxNode == null | (tmp = getStressLevel(sNetwork, sn)) > maxNumber) {
                maxNumber = tmp;
                maxNode = sn;
            }
        }
    ...
    Collection<SubstrateNode> fill_v_a(VirtualNetwork vNetwork,
        ...
        for (int i = 0; i < vNetwork.getVertexCount() - 1; ++i) {
            SubstrateNode minPotentialNode = getMinPiNode(sNetwork, null, v_a, null);
    ...
    LinkedList<Entry<SubstrateNode, Double>> get_nrValuesList(
        ...
        for (SubstrateNode n : v_a) {
            nrValues.add(new Entry<SubstrateNode, Double>(n, getNRValue(sNetwork, n)));
        }
    ...
    boolean mapNodes(Collection<SubstrateNode> v_a, SubstrateNetwork sNetwork,
        ...
        for (Entry<VirtualNode, Integer> e : degrees) {
            ...
            if (!fulfilled) {
                ...
                demandedResources.addAll(Utils.occupyResources(vn.get(), minPiNode.get()));
    ...
    // this methods make it easier to modify the behavior of the algorithm by overwriting them in a subclass

    public double getStressLevel(SubstrateNetwork sNetwork, SubstrateNode n) {
        return Utils.getStressLevel(n);
    }

    public double getStressLevel(SubstrateNetwork sNetwork, SubstrateNode node, SubstrateLink l) {
