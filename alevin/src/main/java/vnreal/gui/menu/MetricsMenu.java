...
import vnreal.evaluations.metrics.AbstractEvaluation;
...
public final class MetricsMenu extends JMenu implements ActionListener {
    ...
    public MetricsMenu() {
        ...
        try {
            for (Class<?> c : ClassScanner.getDerivates(this.getClass(),
                    "vnreal.evaluations.metrics", AbstractEvaluation.class))
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (ToolKit.getScenario().getNetworkStack() != null)
            if (ToolKit.getScenario().getNetworkStack().hasMappings())
                try {
                    new MetricsDialog((AbstractEvaluation) metricClasses.get(
                            cmd).newInstance());
                } catch (Exception e1) {
                    e1.printStackTrace();
                }
