...
public class ScenarioWizard extends AbstractButtonDialog {
    ...
    protected JPanel createContent() {
        ...
        snPanel
                .setBorder(BorderFactory
                        .createTitledBorder("Substrate Network"));
        ...
        snHorizontal.addGroup(snLayout.createParallelGroup().addComponent(
                snNodesLabel).addComponent(snAlphaLabel).addComponent(
                snBetaLabel));
        snHorizontal.addGroup(snLayout.createParallelGroup().addComponent(
                snNodesSpinner, GroupLayout.PREFERRED_SIZE,
        ...
        snVertical.addGroup(snLayout.createParallelGroup(
                GroupLayout.Alignment.CENTER).addComponent(snNodesLabel)
                .addComponent(snNodesSpinner));
        snVertical.addGroup(snLayout.createParallelGroup(
                GroupLayout.Alignment.CENTER).addComponent(snAlphaLabel)
                .addComponent(snAlphaSpinner, GroupLayout.PREFERRED_SIZE,
                        GroupLayout.DEFAULT_SIZE, GroupLayout.PREFERRED_SIZE));
        snVertical.addGroup(snLayout.createParallelGroup(
                GroupLayout.Alignment.CENTER).addComponent(snBetaLabel)
                .addComponent(snBetaSpinner, GroupLayout.PREFERRED_SIZE,
                        GroupLayout.DEFAULT_SIZE, GroupLayout.PREFERRED_SIZE));
                ...
                if (rows < vns)
                    for (int i = rows + 1; i <= vns; i++)
                        tableModel
                                .addRow(new Object[]{
                                        new Integer(i),
                                        (Integer) defaultVNodesNumberSpinner
                                                .getValue(), new Double(1),
                                        new Double(0.5)});
                else
                    for (int i = rows - 1; i >= vns; i--)
        ...
        vnHorizontal.addGroup(vnLayout.createSequentialGroup().addComponent(
                vnNumberLabel).addComponent(vnNumberSpinner,
                GroupLayout.PREFERRED_SIZE, GroupLayout.DEFAULT_SIZE,
                GroupLayout.PREFERRED_SIZE).addPreferredGap(
                LayoutStyle.ComponentPlacement.UNRELATED).addComponent(
                defaultVNodesNumberLabel).addComponent(
                defaultVNodesNumberSpinner, GroupLayout.PREFERRED_SIZE,
                GroupLayout.DEFAULT_SIZE, GroupLayout.PREFERRED_SIZE));
        ...
        vnVertical.addGroup(vnLayout.createParallelGroup(
                GroupLayout.Alignment.CENTER).addComponent(vnNumberLabel)
                .addComponent(vnNumberSpinner).addComponent(
                        defaultVNodesNumberLabel).addComponent(
                        defaultVNodesNumberSpinner));
    ...
    protected void doAction() {
        ...
        generateTopology(substrateNodes, substrateAlpha, substrateBeta,
     ...
     * overrides it!
     *
     * @param snNodes  Number of substrate network nodes.
     * @param snAlpha  The value of alpha for the substrate network.
     * @param snBeta   The value of beta for the substrate network.
     * @param vnNumber The number of virtual networks.
     * @param vnsNodes An array containing the number of nodes for each virtual
     *                 network. vnsNodes[i] is the number of nodes for layer i + 1.
     * @param vnsAlpha An array containing the alpha parameter for each virtual
     *                 network. vnsAlpha[i] is the alpha value for layer i + 1.
     * @param vnsBeta  An array containing the beta parameter for each virtual
     *                 network. vnsBeta[i] is the beta value for layer i + 1.
     */
    public static void generateTopology(int snNodes, double snAlpha,
        ...
        for (int layer = 1; layer <= vnNumber; layer++) {
            ...
            ToolKit.getScenario().getNetworkStack().addLayer(vn);
