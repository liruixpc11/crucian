...
public class AddConstraintDialog extends AbstractAddEditConstraintDialog {
        ...
        if (comboBox.getSelectedIndex() > 0) {
            try {
                ...
                Object cons = ((ComboBoxClass) comboBox.getSelectedItem())
                        .getClasss().getConstructor(
                                entity.getClass().getSuperclass()).newInstance(
                                entity);
                ...
                for (int i = 0; i < paramNames.size(); i++) {
                    ...
                    setter.invoke(cons, getParamValue(paramType[0], paramValues
                            .get(i).getText()));
                }
    ...
    protected Object[] getComboBoxObjects() {
        ...
        try {
            ...
            // the combo box text.
            Class<?>[] classes = ClassScanner.getClassesImplementing(this
                    .getClass(), pckgname, Class.forName(intfaceName));
            ComboBoxClass[] cbObj = new ComboBoxClass[classes.length + 1];
            cbObj[0] = new ComboBoxClass(null, "select " + constraintStr
                    + "...");
            for (int i = 0; i < classes.length; i++) {
                cbObj[i + 1] = new ComboBoxClass(classes[i], classes[i]
                        .getSimpleName());
            }
            return cbObj;
