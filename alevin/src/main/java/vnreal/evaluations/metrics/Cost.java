...
/**
 * Class to obtain the substrateCost incurred in the substrate network
 * after performing a mapping over a set of VNRs.
 *
 * The substrateCost of realizing the embedding of one virtual network request
 ...
 *
 * substrateCost(G^k(V^k,A^k))= \sum_{i^k \in V^k} ND_{cpu}(i^k) + \sum_{(i,j) \in A}\sum_{(i^k,j^k) \in A^k} \rho_{i^k,j^k}(i,j) LD_{bw}(i^k,j^k)
 *
 * where ND_{cpu}(i^k) is the demand of CPU in the node i^k and LD_bw(i^k,j^k) is the 
 * link demand of virtual link (i^k,j^k) belonging to the VNR k. The total substrateCost is
 * the sum of the substrateCost incurred by the SN to map all the VNRs.
 *
 * The substrateCost can be defined as the resources spent in the substrate network to embed the virtual network request.
 *
...
public class Cost extends AbstractEvaluation {
    ...
    /**
     * @return the substrateCost of incurred in the susbtrate network
     * after performing a mapping over a set of VNRs
     */
