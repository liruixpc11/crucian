...
public class RandomEvaluationExperiment {
    public static void main(String[] args) throws IOException {
		...
		final int[] numVNetsArray = { 10,15,20,50 };
						...
						for (int scenarioNumber = 0; scenarioNumber < numScenarios; scenarioNumber++)
							for (int scenarioRun = 0; scenarioRun < numRunsPerScenario; scenarioRun++) {
								scenarioSuffix = numSNodes + "_" + numVNets
										...
										+ scenarioRun;
								new ScenarioImporter(filepath
										+ "Scenario-mapped_" + scenarioSuffix
										+ ".xml").setNetworkStack();
								NetworkStack stack = ToolKit
										.getScenario().getNetworkStack();

								String path = filepath
										+ "Evaluation-results_"
										+ scenarioSuffix + "-" + algoName
										+ ".csv";
								...
								metrics.add(new CostRevTimesMappedRev(false));
								...
								metrics.add(new RemainingLinkResource());
								metrics.add(new LinkCostPerVnr());
								for (Iterator<EvaluationMetric> tmpMetric = metrics
										.iterator(); tmpMetric.hasNext();) {
									EvaluationMetric currMetric = tmpMetric
											.next();
									currMetric.setStack(stack);
								}
								EvaluationFileGeneration outputFile = new EvaluationFileGeneration(
										path);
								outputFile.createEvaluationFile(metrics);

							}*/

/*		GenerationGnuplotFiles test;
		LinkedList<String> algorithms = new LinkedList<String>();
		algorithms.add("RViNEPS");
		algorithms.add("RoundingMP");
		algorithms.add("DViNESP");
		algorithms.add("DViNEPS");
		algorithms.add("RViNESP");
		algorithms.add("GARPS");
		algorithms.add("GARSP");
		test = new GenerationGnuplotFiles(false, false, 20, 10);
		try {
			test.generateFiles(algorithms);
		} catch (IOException e) {
			e.printStackTrace();
		}
		test = new GenerationGnuplotFiles(false, false, 20, 15);
		try {
			test.generateFiles(algorithms);
		} catch (IOException e) {
			e.printStackTrace();
		}
		test = new GenerationGnuplotFiles(false, false, 20, 20);
		try {
			test.generateFiles(algorithms);
		} catch (IOException e) {
			e.printStackTrace();
		}
		test = new GenerationGnuplotFiles(false, false, 20, 50);
		try {
			test.generateFiles(algorithms);
		} catch (IOException e) {
			e.printStackTrace();
		...
		/*GraphsFileGeneration test;
		test = new GraphsFileGeneration("GARSP", false);
		test.generateFiles();
		test = new GraphsFileGeneration("GARPS", false);
		test.generateFiles();
		test = new GraphsFileGeneration("DViNEPS", false);
		test.generateFiles();
		test = new GraphsFileGeneration("DViNESP", false);
		test.generateFiles();
		test = new GraphsFileGeneration("RViNEPS", true);
		test.generateFiles();
		test = new GraphsFileGeneration("RViNESP", true);
		test.generateFiles();
		test = new GraphsFileGeneration("RoundingMP", true);
		test.generateFiles();*/
