...
@Ignore
public class RoundingMP extends AbstractLoadScenarioTest {
    public RoundingMP(ScenarioData data) {
        super(data);
    ...
    public void runAlgorithm() {
		...
		/*
		 * Coordinated node and link mapping with Rounding Multipath
		 * Important Parameters 
		 * Rounding Type: Deterministic rounding (More combinations can be used)
		 * Maximum number of shortest path to calculate: 20
		 * Hidden Hops Considered: False
		 * Distance Value : 40 
		 */

        String path = System.getProperty("user.home") + File.separator + "Evaluation-results_" + scenario_suffix + "-RoundingMP.csv";
        List<EvaluationMetric> metrics = new LinkedList<EvaluationMetric>();
        IAlgorithm algo = new CoordinatedMappingRoundingPathStripping(ToolKit
